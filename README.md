-- markiyanbest's script (V64 - FIXED AUTO FARM FINAL)
...
-- ============================================================
-- ПЕРЕВІРКА: ОБ'ЄКТ ВСЕРЕДИНІ ПЕРСОНАЖА ГРАВЦЯ?
-- ============================================================
...
-- ============================================================
-- ЧОРНИЙ СПИСОК ДІЙ/ПРОМПТІВ (+ rent, house, claim, clan, base)
-- ============================================================
...
-- ============================================================
-- НОВА ПРОСТА СИСТЕМА ФАРМУ
-- Логіка: беремо ТІЛЬКИ якщо ім'я предмета (parent.Name) є в нашому списку
-- і дія = collect/grab/pick up/take/loot/get/steal/pickup або пуста
-- ============================================================

local function IsItemInList(name)
	if not name or name == "" then return false end
	local nl = name:lower():gsub("^%s+",""):gsub("%s+$","")
	if ALL_ITEMS_LOOKUP[nl] then return true end
	-- Часткове співпадіння для скінів
	if IsGunSkin(nl) then return true end
	return false
end
...
-- ★★★ ГОЛОВНА ФУНКЦІЯ ВАЛІДАЦІЇ — ПРОСТА І НАДІЙНА ★★★
local function IsValidLootPrompt(prompt)
	...
	-- ★ КРОК 1: Перевіряємо чорний список дій
	if BLACKLIST_ACTIONS[actionText] then return false end

	-- ★ КРОК 2: Перевіряємо чорний список слів в parent, action, object
	...
	-- ★ КРОК 3: Перевіряємо grandparent на чорний список
	pcall(function()
		if par.Parent then
	...
	-- ★ КРОК 5: Якщо предмет НЕ знайдений в списку — НЕ БЕРЕМО
	if not matchedItem then return false end

	-- ★ КРОК 6: Перевіряємо чи предмет увімкнений в Item Picker
	if not IsItemEnabled(matchedItem) then return false end

	-- ★ КРОК 7: Дія — або пуста, або collect/grab/pick up/take/loot/get/steal/pickup/equip
	...
	-- Перевіряємо часткове співпадіння
...
-- ============================================================
-- AIM SYSTEM (НЕ ПЕРЕКЛЮЧАЄТЬСЯ АВТОМАТИЧНО)
-- ============================================================
local aimTarget = nil
...
-- ============================================================
-- AUTO FARM (ПЕРЕПИСАНИЙ — ТІЛЬКИ ПО СПИСКУ)
-- ============================================================
...
local MW=IsMobile and 300 or 420
local MH=IsMobile and 560 or 660
...
Header.Size=UDim2.new(1,0,0,44); Header.BackgroundColor3=Color3.fromRGB(10,10,20); Header.BorderSizePixel=0
...
TL.HorizontalAlignment=Enum.HorizontalAlignment.Center; TL.VerticalAlignment=Enum.VerticalAlignment.Center; TL.Padding=UDim.new(0,3)

local Scroll=Instance.new("ScrollingFrame",Main)
Scroll.Size=UDim2.new(1,-8,1,-86); Scroll.Position=UDim2.new(0,4,0,82)
...
tI.Size=UDim2.new(0,220,0,24); tI.Position=UDim2.new(0.5,-110,0.5,-(Config.AimFOV+34))
...
tI.Font=Enum.Font.GothamBold; tI.TextSize=11; tI.Text=""; tI.Visible=false; tI.ZIndex=12
...
local function UpdateFOV()
	...
	tI.Position=UDim2.new(0.5,-110,0.5,-(r+34))
end
...
local flyH=Instance.new("Frame",SG); flyH.Size=UDim2.new(0,144,0,64)
flyH.Position=UDim2.new(1,-158,1,-80); flyH.BackgroundTransparency=1; flyH.Visible=false; flyH.ZIndex=50
local function MkFB(l,x,cb)
	local b=Instance.new("TextButton",flyH); b.Size=UDim2.new(0,64,0,60); b.Position=UDim2.new(0,x,0,0)
...
MkFB("▲",0,function(v) MobUp=v end); MkFB("▼",76,function(v) MobDn=v end)
local function UpdateFlyBtns() flyH.Visible=Config.Fly and IsMobile end
UpdateFlyBtns_=UpdateFlyBtns

local scHolder=Instance.new("Frame",SG); scHolder.Size=UDim2.new(0,56,0,580)
scHolder.Position=UDim2.new(1,-62,0.10,0); scHolder.BackgroundTransparency=1; scHolder.BorderSizePixel=0; scHolder.ZIndex=90
local scLayout=Instance.new("UIListLayout",scHolder); scLayout.Padding=UDim.new(0,5); scLayout.HorizontalAlignment=Enum.HorizontalAlignment.Center
...
for _, def in ipairs(ShortcutDefs) do
	local btn=Instance.new("TextButton",scHolder); btn.Size=UDim2.new(0,52,0,42)
	btn.BackgroundColor3=Color3.fromRGB(20,20,30); btn.TextColor3=Color3.fromRGB(180,180,190)
	btn.Font=Enum.Font.GothamBlack; btn.TextSize=IsMobile and 11 or 10; btn.Text=def.label
	...
	if def.key=="_SafeTP" then
		...
		btn.MouseButton1Click:Connect(function()
			...
			if def.key=="AimActive" then aimTarget=nil; aimLocked=false; aimLostFrames=0; aimHasLockedOnce=not Config.AimActive and true or false end
...
local tabW=IsMobile and 48 or 64

for _, n in pairs(tabNames) do
	Sections[n]={}
	local b=Instance.new("TextButton",TabBar); b.Size=UDim2.new(0,tabW,0,24)
...
local function AddCategory(tab,text)
	local f=Instance.new("Frame",Scroll); f.Size=UDim2.new(0.97,0,0,22)
	...
	l.TextColor3=Color3.new(1,1,1); l.Font=Enum.Font.GothamBold; l.TextSize=IsMobile and 10 or 11
	l.Text="── "..text.." ──"
...
local function AddToggle(tab,name,key,cbOn,cbOff)
	...
	btn.Font=Enum.Font.GothamBold; btn.TextSize=IsMobile and 12 or 13
	...
	local dot=Instance.new("Frame",btn); dot.Size=UDim2.new(0,9,0,9); dot.AnchorPoint=Vector2.new(0,0.5)
	dot.Position=UDim2.new(0,10,0.5,0); dot.BackgroundColor3=Color3.fromRGB(200,50,50)
...
local function AddSlider(tab,label,minV,maxV,def,cKey,cb)
	local f=Instance.new("Frame",Scroll); f.Size=UDim2.new(0.97,0,0,IsMobile and 56 or 54)
	...
	local lbl=Instance.new("TextLabel",f); lbl.Size=UDim2.new(1,-8,0,22); lbl.Position=UDim2.new(0,4,0,2)
	...
	lbl.Text=label..": "..cv
	local tr=Instance.new("Frame",f); tr.Size=UDim2.new(0.92,0,0,IsMobile and 14 or 10)
	tr.Position=UDim2.new(0.04,0,0,IsMobile and 36 or 36)
...
local function AddAction(tab,name,color,cb)
	...
	btn.TextColor3=Color3.new(1,1,1); btn.Font=Enum.Font.GothamBold; btn.TextSize=IsMobile and 12 or 13
...
local function AddTP(tab,name,vec)
	...
	btn.Font=Enum.Font.GothamBold; btn.TextSize=IsMobile and 12 or 12
	btn.BorderSizePixel=0; btn.AutoButtonColor=false; btn.Text="📍 "..name; Instance.new("UICorner",btn)
...
local iTotalLabel=Instance.new("Frame",Scroll); iTotalLabel.Size=UDim2.new(0.97,0,0,28)
...
iTL.TextColor3=Color3.new(1,1,1); iTL.Font=Enum.Font.GothamBold; iTL.TextSize=IsMobile and 10 or 11
iTL.Text="📦 ITEM PICKER — "..#ALL_ITEMS.." items | ⭐=priority"

local farmStatsLabel=Instance.new("Frame",Scroll); farmStatsLabel.Size=UDim2.new(0.97,0,0,24)
...
fSL.TextColor3=Color3.fromRGB(150,255,150); fSL.Font=Enum.Font.Gotham; fSL.TextSize=10
...
local sFr=Instance.new("Frame",Scroll); sFr.Size=UDim2.new(0.97,0,0,IsMobile and 42 or 36)
...
local sB=Instance.new("TextBox",sFr); sB.Size=UDim2.new(0.55,-4,0,IsMobile and 30 or 26)
sB.Position=UDim2.new(0,6,0.5,IsMobile and -15 or -13); sB.BackgroundColor3=Color3.fromRGB(25,25,40)
sB.TextColor3=Color3.new(1,1,1); sB.PlaceholderText="🔍 search..."
sB.PlaceholderColor3=Color3.fromRGB(100,100,130); sB.Font=Enum.Font.Gotham; sB.TextSize=IsMobile and 12 or 12
sB.ClearTextOnFocus=false; sB.BorderSizePixel=0; Instance.new("UICorner",sB)
local eA=Instance.new("TextButton",sFr); eA.Size=UDim2.new(0.21,0,0,IsMobile and 30 or 26)
eA.Position=UDim2.new(0.57,2,0.5,IsMobile and -15 or -13); eA.BackgroundColor3=Color3.fromRGB(0,120,50)
eA.TextColor3=Color3.new(1,1,1); eA.Font=Enum.Font.GothamBold; eA.TextSize=10; eA.Text="ALL✓"; eA.BorderSizePixel=0
Instance.new("UICorner",eA)
local dAB=Instance.new("TextButton",sFr); dAB.Size=UDim2.new(0.21,0,0,IsMobile and 30 or 26)
dAB.Position=UDim2.new(0.79,2,0.5,IsMobile and -15 or -13); dAB.BackgroundColor3=Color3.fromRGB(150,30,30)
dAB.TextColor3=Color3.new(1,1,1); dAB.Font=Enum.Font.GothamBold; dAB.TextSize=10; dAB.Text="ALL✗"; dAB.BorderSizePixel=0
Instance.new("UICorner",dAB)

local itemBtns={}

for _, cat in ipairs(ItemCategories) do
	local catF=Instance.new("Frame",Scroll); catF.Size=UDim2.new(0.97,0,0,IsMobile and 32 or 28)
	...
	catLbl.Font=Enum.Font.GothamBold; catLbl.TextSize=IsMobile and 10 or 11; catLbl.TextXAlignment=Enum.TextXAlignment.Left
	catLbl.Text=cat.name.." ("..#cat.items..")"

	local catOn=Instance.new("TextButton",catF); catOn.Size=UDim2.new(0.11,0,0,IsMobile and 22 or 20)
	catOn.Position=UDim2.new(0.76,0,0.5,IsMobile and -11 or -10); catOn.BackgroundColor3=Color3.fromRGB(0,100,40)
	catOn.TextColor3=Color3.new(1,1,1); catOn.Font=Enum.Font.GothamBold; catOn.TextSize=9; catOn.Text="✓"; catOn.BorderSizePixel=0
	Instance.new("UICorner",catOn)
	local catOff=Instance.new("TextButton",catF); catOff.Size=UDim2.new(0.11,0,0,IsMobile and 22 or 20)
	catOff.Position=UDim2.new(0.88,0,0.5,IsMobile and -11 or -10); catOff.BackgroundColor3=Color3.fromRGB(120,20,20)
	catOff.TextColor3=Color3.new(1,1,1); catOff.Font=Enum.Font.GothamBold; catOff.TextSize=9; catOff.Text="✗"; catOff.BorderSizePixel=0
	Instance.new("UICorner",catOff)

	local catItemBtns={}
	for _, iN in ipairs(cat.items) do
		if ItemPickerState[iN]==nil then ItemPickerState[iN]=true end
		local f=Instance.new("Frame",Scroll); f.Size=UDim2.new(0.97,0,0,IsMobile and 32 or 28)
		f.BackgroundTransparency=1; f.BorderSizePixel=0; f.Visible=false
...
local function AddBR(tab,aK,aN)
	local f=Instance.new("Frame",Scroll); f.Size=UDim2.new(0.97,0,0,IsMobile and 44 or 38)
	...
	nl.Font=Enum.Font.GothamBold; nl.TextSize=IsMobile and 12 or 13; nl.TextXAlignment=Enum.TextXAlignment.Left; nl.Text=aN
	local bb=Instance.new("TextButton",f); bb.Size=UDim2.new(0.42,0,0,IsMobile and 30 or 26)
	bb.Position=UDim2.new(0.55,0,0.5,IsMobile and -15 or -13); bb.BackgroundColor3=Color3.fromRGB(22,22,38)
	bb.TextColor3=Color3.fromRGB(170,200,255); bb.Font=Enum.Font.GothamBold; bb.TextSize=IsMobile and 11 or 11
...
local scInfo=Instance.new("Frame",Scroll); scInfo.Size=UDim2.new(0.97,0,0,IsMobile and 30 or 24)
...
scInfoL.TextColor3=Color3.fromRGB(120,160,255); scInfoL.Font=Enum.Font.Gotham; scInfoL.TextSize=10; scInfoL.TextWrapped=true
scInfoL.Text="Show/hide screen buttons"

for _, def in ipairs(ShortcutDefs) do
	local f=Instance.new("Frame",Scroll); f.Size=UDim2.new(0.97,0,0,IsMobile and 38 or 34)
	...
	btn.TextSize=IsMobile and 11 or 12; btn.BorderSizePixel=0; btn.AutoButtonColor=false
...
local MS=IsMobile and 56 or 44
local MB=Instance.new("TextButton",SG); MB.Size=UDim2.new(0,MS,0,MS); MB.Position=UDim2.new(0,10,0.25,0)
MB.Text="M"; MB.Font=Enum.Font.GothamBlack; MB.TextSize=IsMobile and 24 or 20
...
Notify("⚡ V64","Farm: ONLY list items | F blocked | Aim fixed ✓",5)
